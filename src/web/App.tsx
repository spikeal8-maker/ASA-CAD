...
export function App(props: CadProjectPersistenceOverrides) {
  ...
  const uiActions = useM2CadUiActions({open: openLocal,save: saveLocal,undo,redo,rebuild,createSketch: beginCreateSketch,line: beginLine,rectangle: beginRectangle,circle: beginCircle,arc: beginArc,deleteSketchEntity: deleteSelectedSketchEntity,horizontalConstraint: applyHorizontalConstraint,verticalConstraint: applyVerticalConstraint,fixedConstraint: applyFixedConstraint,coincidentConstraint: workspace.beginCoincidentConstraint,parallelConstraint: workspace.beginParallelConstraint,perpendicularConstraint: workspace.beginPerpendicularConstraint, tangentConstraint: workspace.beginTangentConstraint, concentricConstraint: workspace.beginConcentricConstraint, equalConstraint: workspace.beginEqualConstraint, symmetricConstraint: workspace.beginSymmetryConstraint, pointOnCurveConstraint: workspace.beginPointOnCurveConstraint, finishSketch, extrude: beginExtrude, cutExtrude: beginCut, fillet: beginFillet, fit: () => requestView('Показать всё'), front: () => requestView('Спереди'), back: () => requestView('Сзади'), top: () => requestView('Сверху'), bottom: () => requestView('Снизу'), left: () => requestView('Слева'), right: () => requestView('Справа'), isometric: () => requestView('Изометрия'),},{ canUndo: state.canUndo, canRedo: state.canRedo, hasSketch: Boolean(sketch), hasSketchEntitySelection: Boolean(selectedSketchEntityId), canApplyOrientationConstraint, canApplyFixedConstraint, canApplyCoincidentConstraint: workspace.canApplyCoincidentConstraint, canApplyParallelConstraint: workspace.canApplyParallelConstraint, canApplyPerpendicularConstraint: workspace.canApplyPerpendicularConstraint, canApplyConcentricConstraint: workspace.canApplyConcentricConstraint, canApplyEqualConstraint: workspace.canApplyEqualConstraint, canApplySymmetryConstraint: workspace.canApplySymmetryConstraint, canApplyPointOnCurveConstraint: workspace.canApplyPointOnCurveConstraint, canApplyTangentConstraint: workspace.canApplyTangentConstraint, canExtrude, canCutExtrude: canCut, canFillet,},); const searchableActions=uiActions.search(search);
  ...
  async function dispatchShortcutAction(action: ShortcutActionId) {
    ...
    if (sharedActionId) {
      ...
      await sharedAction.execute(); return;
    }
    switch (action) {
      case 'interaction.cancel':
        if (activeCommand && selectedPick) { clearSelectedPick(); setNotice('Выбор очищен; команда остаётся активной'); return; }
        if (activeCommand) { cancelCommand(); return; }
        clearTransientSelection(); setNotice('Выбор очищен'); return;
