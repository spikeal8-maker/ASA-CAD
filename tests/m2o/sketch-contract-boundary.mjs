...
assert.doesNotMatch(solver, /as Record<string, unknown>/, 'PlaneGCS adapter must consume typed constraint/entity DTOs directly');
assert.match(solver, /case 'arc':/, 'PlaneGCS typed entity boundary must retain supported arc geometry');
assert.match(solver, /case 'parallel':/, 'PlaneGCS typed constraint boundary must retain supported parallel constraint');
assert.match(solver, /case 'angular':/, 'PlaneGCS typed dimension boundary must retain supported angular dimension');
