...
const handlers: M2CadUiActionHandlers = {
  ...
  constructionToggle: call('constructionToggle'),
  ...
  angularDimension: call('angularDimension'),
...
assert.equal(actions.get('sketch.construction')?.enabled, true);
...
assert.equal(actions.get('dimension.angular')?.enabled, true);
...
const disabledConstraints = indexCadUiActions(createCadUiActions(definitions, createM2CadUiActionBindings(handlers, {
  ...
  canApplyAngularDimension: false,
  canExtrude: false,
  canCutExtrude: false,
  canFillet: false,
})));
assert.equal(disabledConstraints.get('sketch.construction')?.enabled, false);
...
assert.equal(disabledConstraints.get('dimension.angular')?.enabled, false);
assert.equal(disabledConstraints.get('dimension.angular')?.disabledReason, 'Создайте два отрезка эскиза');
