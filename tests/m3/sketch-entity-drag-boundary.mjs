...
const selectionCss = fs.readFileSync('src/web/viewport/sketch-selection.css', 'utf8');
...
assert.match(selectionCss, /\.cad-sketch-selection-hit-target\s*\{[\s\S]*touch-action:\s*none;/,
  'selected Sketch hit targets must keep one-finger touch drag in the Pointer Events stream');
...
console.log('ASA-CAD M3.6B drag architecture PASS (typed stable-ID command + transient solver candidate + focused mouse/touch interaction owner)');
