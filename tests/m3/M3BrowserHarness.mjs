...
export async function activateMobileTool(page, commandId, label) {
  ...
  const tools = page.locator('[data-mobile-tools="true"]');
  await tools.waitFor();
  const button = tools.locator(`[data-command-id="${commandId}"]`);
...
export function assertNoPageErrors(errors, label) {
...
export async function openFixture(browser, fixture, entityCount) {
  ...
  const overlay = await waitSolvedOverlay(page, entityCount, `${fixture} fixture`);
  return { page, overlay };
}
