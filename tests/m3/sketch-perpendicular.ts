...
const pairOwner = fs.readFileSync('src/application/commands/SketchLinePairConstraintOwner.ts', 'utf8');
...
assert.match(handler, /'constraint\.perpendicular'/, 'constraint handler map must register Perpendicular');
assert.match(handler, /addLinePairConstraint/, 'constraint handler map must delegate pair semantics to the focused owner');
assert.doesNotMatch(handler, /sameUnorderedEntityPair/, 'general constraint owner must not reabsorb Line-pair duplicate logic');
assert.match(pairOwner, /sameUnorderedEntityPair/, 'focused Line-pair owner must reject symmetric duplicates');
assert.match(pairOwner, /requires two Line entities/, 'focused Line-pair owner must enforce Line-only scope');
assert.doesNotMatch(`${handler}\n${pairOwner}`, /PlaneGCS|OpenCascade|vendor\//, 'application constraint owners must remain runtime-neutral');
...
solver.dispose();
app.dispose();
console.log('ASA-CAD M3.7E Perpendicular PASS (Line pair + duplicate/type/scope guards + PlaneGCS + persistence/history + focused UI/application ownership)');
