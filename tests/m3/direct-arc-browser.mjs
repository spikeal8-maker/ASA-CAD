...
async function desktopDirectArc() {
  ...
  const radiusGuide = page.locator('[data-testid="sketch-arc-radius"]');
  await radiusGuide.waitFor({ state: 'attached' });
  const radiusCoords = await Promise.all(['x1', 'y1', 'x2', 'y2'].map((name) => radiusGuide.getAttribute(name)));
  near(
    Math.hypot(
      Number(radiusCoords[2]) - Number(radiusCoords[0]),
      Number(radiusCoords[3]) - Number(radiusCoords[1]),
    ),
    12,
    0.8,
    'radius guide length',
