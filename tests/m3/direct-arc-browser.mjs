...
async function waitSolvedArc(page) {
  ...
  return overlay.locator('path[data-sketch-entity-id][data-sketch-entity-type="arc"]').first();
}

async function assertArcGeometry(path, { radius, startAngle, endAngle }, tolerance = 0.3) {
  near(Number(await path.getAttribute('data-arc-radius')), radius, tolerance, 'arc radius');
  near(Number(await path.getAttribute('data-arc-start-angle')), startAngle, tolerance, 'arc start angle');
  near(Number(await path.getAttribute('data-arc-end-angle')), endAngle, tolerance, 'arc end angle');
  assert.ok((await path.getAttribute('d'))?.startsWith('M '), 'Arc overlay has no SVG path');
}
...
async function deterministicArcFixture() {
  ...
  await page.getByText('Fixture arc: R15, 90° CCW, центр (0, 0) в XY', { exact: true }).waitFor();
  ...
  await assertArcGeometry(arc, { radius: 15, startAngle: 0, endAngle: Math.PI / 2 }, 0.2);
...
async function desktopDirectArc() {
  ...
  await page.locator('[data-testid="sketch-arc-radius-preview"]').waitFor();
  await page.mouse.click(start.x, start.y);
  await page.locator('[data-testid="cad-sketch-interaction"][data-arc-phase="sweep"]').waitFor();
  await page.locator('[data-testid="sketch-arc-start"]').waitFor();
  ...
  const persistedArc = await waitSolvedArc(page);
  await assertArcGeometry(persistedArc, { radius: 12, startAngle: 0, endAngle: Math.PI / 2 }, 0.8);
  ...
  near(entity.data.radius, 12, 1.0, 'saved radius');
...
async function touchDirectArc() {
  ...
  try {
    ...
    const center = squarePoint(box, 0.46, 0.52);
    const start = squarePoint(box, 0.61, 0.52);
    const end = squarePoint(box, 0.46, 0.37);
    ...
    console.log('  ✓ touch Arc: three taps use shared surface and commit one typed Arc');
  } finally {
    await context.close();
  }
}
