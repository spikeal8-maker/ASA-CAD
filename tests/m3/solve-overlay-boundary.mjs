...
const statusBar = readFileSync('src/web/StatusBar.tsx', 'utf8');
...
assert.match(app, /<StatusBar/, 'App must render the focused status owner');
assert.match(app, /solveSnapshot=\{solveSnapshot\}/, 'App must pass solve state to the focused status owner');
assert.match(statusBar, /<SketchSolveStatus/, 'StatusBar must own the explicit solve-status projection');
...
assert.doesNotMatch(
  ...
  'transient solve session must not import persisted application history ownership',
);
assert.doesNotMatch(solveSession, /\.execute\(/, 'transient solve session must not persist solved preview through hidden commands');
